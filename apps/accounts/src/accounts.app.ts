...
const app = new Hono<App>()
	...
	.put(
		...
		async (c) => {
			const id = await authedId(c)
			if (id === null) return unauthorized(c)
			const rawIdentityFlags = (await formField(c, 'identityFlags')).trim()
			if (!/^-?\d+$/.test(rawIdentityFlags)) return c.body(null, 400)
			const identityFlags = Number(rawIdentityFlags)
			if (!Number.isSafeInteger(identityFlags)) return c.body(null, 400)
